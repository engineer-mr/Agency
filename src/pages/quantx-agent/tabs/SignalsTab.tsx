...
function SwitchRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-4">
      ...
      >
        <span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow-sm transition ${value ? 'left-6' : 'left-1'}`} />
      </button>
    </div>
  )
}
      ...
      <Dialog.Portal>
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              <Dialog.Title className="text-[22px] font-semibold tracking-tight text-slate-900">{t('quantxAgent.riskDialog.title')}</Dialog.Title>
          ...
          <div className="mt-6 space-y-4">
            {[
              { label: t('quantxAgent.control.limit'), value: limit, onChange: onLimitChange, suffix: 'USDT', type: 'number' as const },
              { label: t('quantxAgent.control.dailyLoss'), value: dailyLoss, onChange: onDailyLossChange, suffix: 'USDT', type: 'number' as const },
              { label: t('quantxAgent.control.drawdown'), value: drawdown, onChange: onDrawdownChange, suffix: '%', type: 'number' as const },
            ].map((field) => (
              ...
              <div className="space-y-5">
                  ...
                  <div className="divide-y divide-slate-100">
                    {signals.map((signal) => (
                      ...
                      >
                        ...
                        <SignalScore risk={signal.risk} score={signal.score} overallLabel={overallScoreLabel} tone={signal.scoreTone} />
                        <Button
                          type="button"
                      ...
                      <article key={agent.title} className="flex min-h-[230px] flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-xl 
                           bg-[#eef2fb] text-[#0f4cc8]">✦</div>
                          <div className="flex items-baseline gap-1">
                            <span className="text-sm font-semibold text-slate-500">{splitScore(agent.score).label}</span>
                            <span className="relative top-[0.1rem] text-2xl font-semibold tracking-tight text-slate-900">
                              {splitScore(agent.score).value}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="mt-auto pt-5">
                          ...
                          <Button
                            type="button"
                            onClick={() => navigate('/quantx-agent/order')}
                            className="mt-5 flex items-center justify-center text-sm font-semibold text-[#0f4cc8]"
                          >
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-end justify-between gap-4 border-b border-slate-100 pb-4">
                    ...
                    <Button
                      ...
                      className="flex items-center justify-center text-sm font-semibold text-[#0f4cc8]"
                    >
                        ...
                        {queueRows.map((row) => (
                          <tr key={`${row.strategy}-${row.symbol}`} className="border-b border-slate-100 last:border-b-0">
                            ...
                            <td className="py-5 text-right">
                              <span className="rounded-full bg-[#eef2fb] px-3 py-1 text-sm font-semibold text-[#0f4cc8]">{row.status}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                <div className="rounded-2xl bg-[#eef3ff] px-5 py-4 text-sm text-slate-500 shadow-sm">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <Button type="button" className="flex items-center justify-center text-sm font-semibold text-[#0f4cc8]">
              ...
              <div className="space-y-5">
                <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="rounded-full bg-[#eef2fb] px-3 py-1 text-sm font-semibold text-[#0f4cc8]">{t('quantxAgent.control.mode')}</span>
                  </div>

                  <div className="mt-5 border-y border-slate-100">
                    <SwitchRow title={t('quantxAgent.control.entrySignal')} desc={t('quantxAgent.control.entrySignalDesc')} value={receiveSignals} onChange={setReceiveSignals} />
                    <SwitchRow title={t('quantxAgent.control.autoOrder')} desc={t('quantxAgent.control.autoOrderDesc')} value={autoOrder} onChange={setAutoOrder} />
                  </div>
                  ...
                  <div className="mt-4 rounded-2xl bg-[#eef3ff] px-4 py-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <Button type="button" onClick={() => setRiskDialogOpen(true)} className="text-sm font-semibold text-[#0f4cc8]">
                  ...
                  <Button type="button" className="mt-4 flex h-12 w-full items-center justify-center rounded-xl bg-[#0f4cc8] text-sm font-semibold text-white shadow-sm transition hover:bg-[#123fa4]">
                ...
                <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="rounded-full bg-[#f0fbf5] px-3 py-1 text-sm font-semibold text-[#24b07b]">{t('quantxAgent.settlement.safe')}</span>
                  </div>

                  <div className="mt-5 flex items-center gap-3 rounded-2xl border border-slate-100 px-4 py-4">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#f7bf32] text-lg font-bold text-white">B</div>
